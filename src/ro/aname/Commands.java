...
//            }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (command.getName().equalsIgnoreCase("antibot-ultra")) {
            if (args.length == 0) {
                sender.sendMessage("-> /antibot-ultra add <player>: Adds the <player> to the whitelis.t");
                sender.sendMessage("-> /antibot-ultra remove <player>: Removes the <player> from the whitelist.");
                sender.sendMessage("-> /antibot-ultra reload: Reloads the config file.");
                return true;
            }
            if (args[0].equalsIgnoreCase("add")) {
                if (args.length == 1) {
                    sender.sendMessage("You must define a player.");
                    return true;
                }
                Player target = Bukkit.getServer().getPlayer(args[1]);
                if (whitelistManager.isOnWhitelist(target)) {
                    sender.sendMessage(target + " is already on the whitelist.");
                    return true;
                }
                if (target == null) {
                ...
                whitelistManager.setWhitelist(target);
                whitelistManager.saveCustomConfig();
                sender.sendMessage(target + " will be added to the whitelist in " + configHandler.getCustomConfig().getLong("Whitelist.add-delay") + " minute/s.");
                ...
                sender.sendMessage("Invalid commands.");
            }
        }
        return true;
    }
